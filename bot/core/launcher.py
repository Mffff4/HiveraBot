...
START_TEXT = """[red]ВНИМАНИЕ: Эта ферма не предназначена для продажи!
WARNING: This farm is not for sale!
¡ADVERTENCIA: ¡Esta granja no está a la venta!
ATTENTION: Cette ferme n'est pas à vendre!
ACHTUNG: Diese Farm ist nicht zum Verkauf bestimmt!
ATTENZIONE: Questa fattoria non è in vendita!
注意：この農場は販売用ではありません！
주의: 이 농장은 판매용이 아닙니다!
注意：此农场不用于销售！
ATENÇÃO: Esta fazenda não se destina à venda![/red]
...
[cyan]Select action:[/cyan]
    ...
    [green]1. Launch clicker[/green]
    [green]2. Create session[/green]
    [green]3. Create session via QR[/green]
    [green]4. Upload sessions via web (BETA)[/green]

[cyan]Developed by: @Mffff4[/cyan]
[cyan]Our Telegram channel: [blue]https://t.me/+FwTDo63rE4lmMTYy[/blue][/cyan]
"""
...
def prompt_user_action() -> int:
    ...
    while True:
        ...
        if action.isdigit() and action in ("1", "2", "3", "4"):
            return int(action)
        console.print("[yellow]Invalid action. Please enter a number between 1 and 4.[/yellow]")
...
async def run_tasks() -> None:
    ...
    tasks.extend(
        asyncio.create_task(run_tapper(tg_client=tg_client)) 
